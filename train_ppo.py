...
SYSTEM_PROMPT = (
    "你是一个数学解题助手。请严格按照以下格式回答问题：\n"
    "1. 必须包含详细的推理步骤。\n"
    "2. 推理结束后，最后一行必须严格按照 '#### <数字>' 的格式输出答案。\n\n"
    "示例：\n"
    "问题：小明有3个苹果，吃掉了1个，妈妈又给了他2个，现在他有几个？\n"
    "回答：\n"
    "首先，小明原本有3个苹果，吃掉1个后，剩下 3 - 1 = 2 个。\n"
    "然后，妈妈又给了他2个，所以现在共有 2 + 2 = 4 个。\n"
    "#### 4\n\n"
    "现在请回答以下问题："
)
...
def main():
    ...
    generation_kwargs = dict(
        ...
        do_sample=True,
        top_p=0.9,
        temperature=0.5,  # 从 0.7 降低到 0.5，减少 0.5B 小模型的发散和胡言乱语
        pad_token_id=tokenizer.pad_token_id,
        eos_token_id=tokenizer.eos_token_id,
    )
